...
public class AddDateServlet extends HttpServlet {
    ...
    throws ServletException, IOException {
        ...
        String thing=new String(request.getParameter("thing") );
        String date="20"+year+"-"+month+"-"+year;
        if(year.length()==0||month.length()==0||day.length()==0){
            wrong1();
            response.sendRedirect("http://localhost:8080/JRMS/dateManager/addDate.jsp");
            ...
            try{
                ...
                con.close();
                right();
                response.sendRedirect("http://localhost:8080/JRMS/dateManager/lookDate.jsp");
            }catch(Exception e){
                e.printStackTrace();
            }
        }
