...
public class UpdateFriendMessageServlet extends HttpServlet {
    ...
    throws ServletException, IOException {
        ...
        String workPlace=new String(request.getParameter("workPlace") );
        ...
        if(phone.length()==0||email.length()==0||workPlace.length()==0||place.length()==0||QQ.length()==0){
            wrong1();
            response.sendRedirect("http://localhost:8080/JRMS/friendManager/updateFriendMessage.jsp");
        }else{
            try{
                ...
                con.close();
                right();
                response.sendRedirect("http://localhost:8080/JRMS/LookFriendServlet");
            }catch(Exception e){
                e.printStackTrace();
            }
        }
